...
- **只填 Key 才会动配置**：没配 Key 时插件什么都不写。
- **幂等**：重复运行不会重复写入；没有变更时连文件都不写。
- **不覆盖用户配置**：已有档位的模型默认跳过（状态里标为「用户配置」），需要覆盖时显式传 `overwrite_levels: true`。
- **拉不到模型列表就不写模型**：避免落下一堆猜出来的模型 id。
- **每次写入前备份**：`config.json.deepseek-plugin.bak`。

## 常见问题

1. **填了 Key，模型没出现** — 配置在 ZCode 启动时读取，需要重启（或新开会话）。也可先 `deepseek_status` 确认是否已写入。
